...
- **Text feature stack** – Azure/Whisper transcripts, word-aligned timestamps,
  text embeddings, and LLM topic votes enrich the signal surface. We run the LLM
  in sliding 8–12 minute windows with overlap so late-show transitions still get
  context and repeated boundaries can vote before refinement.
- **Diarization cues** – Speaker-change spikes and anchor returns are treated as
  high-confidence boundary hints, especially on CKNW-style shows heavy on live hits.
- **Smart merging** – Post-pass heuristics fuse adjacent clips when gaps are tiny, embeddings stay high, or the LLM type says it's not a new story.
- **LLM story spans**  `{stories: [{start_s, end_s, headline, quote, type, confidence}]}` reduces fragmentation while still giving verbatim quotes and deterministic snapping.
...
2. **Dual feature extraction** – Audio track (VAD, diarization, embeddings) and
   text track (transcription, chunking, embeddings) run in parallel. Topic
   segmentation is handled by overlapping sliding windows so each boundary gets
   multiple chances to be detected.
...
codex-audio segment samples/snare.wav \
  ...
  --out out/cknw_demo
# Add --debug-output (and optional --debug-output-dir) to emit `debug/<station>/<input>/`\n# bundles with per-story JSON for inspection.
```
Produces:
```
out/cknw_demo/
...
With `--debug-output` (or `debug.enabled: true` in the station config), the pipeline also writes debug bundles under `debug/<station>/<input>/` containing `index.json`, `stories.ndjson`, and per-story JSON files so you can inspect the LLM spans, boundaries, and resulting clips.
